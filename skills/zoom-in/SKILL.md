---
name: zoom-in
description: Answer specific questions about code features, implementation details, and architecture decisions by delegating read-only investigation to the Explore subagent. Use when the user asks "How does X work?", "Where is Y implemented?", "Why was X designed this way?", or needs details about a specific feature or function.
---

# Zoom In

Answer specific architecture and implementation questions about code by using the `Explore` subagent to gather focused context, then synthesize a direct answer.
...
2. **Invoke `Explore`** — use the Agent tool with `agentName=Explore` and ask for the narrowest useful thoroughness
...
- Ask `Explore` to trace the main entry point and 2-3 levels of the relevant call flow
...
- Ask `Explore` to find the symbol, its owning module, and the most relevant call sites
...
- Ask `Explore` to find the code implementing the decision and any local signals of intent
