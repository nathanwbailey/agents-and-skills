...
This skill orchestrates three others: an inline mining pass (see step 1), the `write-a-skill` skill (authoring), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.
...
Locate the active workspace's transcripts before fanning out. The workspace's transcripts are in `~/.claude/projects/<slug>/`, where `<slug>` is the workspace path with every non-alphanumeric character turned into `-`. Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Use the `write-a-skill` skill to author the skill. Placement:
