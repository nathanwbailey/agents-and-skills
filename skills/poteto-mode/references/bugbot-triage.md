...
### Later-PR or stack-local usage Bugbot cannot see

- Confidence: candidate
- Skip when: Bugbot flags an export, component, helper, or file as unused, and the PR list and diffs, later-PR diffs, or PR context show it is used by a later PR in the stack.
- Do not skip when: The current PR is not part of a stack, the symbol is public API, or the supposed later-PR use cannot be verified.
- Example signal: "Exported component is never used" with a human reply like "used in the next PR".
